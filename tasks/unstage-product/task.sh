...
RELEASE_NAME=`$CMD -t https://$OPS_MGR_HOST -u $OPS_MGR_USR -p $OPS_MGR_PWD -k curl -s -p /api/v0/available_products | jq --arg product_name $PRODUCT_NAME '.[] | select(.name == $product_name)'`
