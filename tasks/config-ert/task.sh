...
properties_config=$($JQ_CMD -n \
  ...
  --arg properties_autoscale_api_instance_count "$AUTOSCALE_API_INSTANCE_COUNT" \
  --arg properties_autoscale_instance_count "$AUTOSCALE_INSTANCE_COUNT" \
  --arg properties_autoscale_metric_bucket_count "$AUTOSCALE_METRIC_BUCKET_COUNT" \
  --arg properties_autoscale_scaling_interval_in_seconds "$AUTOSCALE_SCALING_INTERVAL_IN_SECONDS" \
  --arg properties_cc_api_rate_limit "$CC_API_RATE_LIMIT" \
  --arg properties_cc_api_rate_limit_enable_general_limit "$CC_API_RATE_LIMIT_ENABLE_GENERAL_LIMIT" \
  --arg properties_cc_api_rate_limit_enable_unauthenticated_limit "$CC_API_RATE_LIMIT_ENABLE_UNAUTHENTICATED_LIMIT" \
  --arg properties_cf_dial_timeout_in_seconds "$CF_DIAL_TIMEOUT_IN_SECONDS" \
  --arg properties_cf_networking_enable_space_developer_self_service "$CF_NETWORKING_ENABLE_SPACE_DEVELOPER_SELF_SERVICE" \
  --arg properties_container_networking "$CONTAINER_NETWORKING" \
  --arg properties_container_networking_interface_plugin "$CONTAINER_NETWORKING_INTERFACE_PLUGIN" \
  --arg properties_container_networking_interface_plugin_silk_dns_servers "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_DNS_SERVERS" \
  --arg properties_container_networking_interface_plugin_silk_enable_log_traffic "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_ENABLE_LOG_TRAFFIC" \
  --arg properties_container_networking_interface_plugin_silk_iptables_accepted_udp_logs_per_sec "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_IPTABLES_ACCEPTED_UDP_LOGS_PER_SEC" \
  --arg properties_container_networking_interface_plugin_silk_iptables_denied_logs_per_sec "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_IPTABLES_DENIED_LOGS_PER_SEC" \
  --arg properties_container_networking_interface_plugin_silk_network_cidr "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_NETWORK_CIDR" \
  --arg properties_container_networking_interface_plugin_silk_network_mtu "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_NETWORK_MTU" \
  --arg properties_container_networking_interface_plugin_silk_vtep_port "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_VTEP_PORT" \
  --arg properties_credhub_database "$CREDHUB_DATABASE" \
  --arg properties_credhub_database_external_host "$CREDHUB_DATABASE_EXTERNAL_HOST" \
  --arg properties_credhub_database_external_password "$CREDHUB_DATABASE_EXTERNAL_PASSWORD" \
  --arg properties_credhub_database_external_port "$CREDHUB_DATABASE_EXTERNAL_PORT" \
  --arg properties_credhub_database_external_tls_ca "$CREDHUB_DATABASE_EXTERNAL_TLS_CA" \
  --arg properties_credhub_database_external_username "$CREDHUB_DATABASE_EXTERNAL_USERNAME" \
  --arg properties_credhub_hsm_provider_client_certificate "$CREDHUB_HSM_PROVIDER_CLIENT_CERTIFICATE" \
  --arg properties_credhub_hsm_provider_client_private_key "$CREDHUB_HSM_PROVIDER_CLIENT_PRIVATE_KEY" \
  --arg properties_credhub_hsm_provider_partition "$CREDHUB_HSM_PROVIDER_PARTITION" \
  --arg properties_credhub_hsm_provider_partition_password "$CREDHUB_HSM_PROVIDER_PARTITION_PASSWORD" \
  --arg properties_credhub_hsm_provider_servers "$CREDHUB_HSM_PROVIDER_SERVERS" \
  --arg properties_credhub_key_encryption_name "$CREDHUB_KEY_ENCRYPTION_NAME" \
  --arg properties_credhub_key_encryption_secret "$CREDHUB_KEY_ENCRYPTION_SECRET" \
  --arg properties_credhub_key_encryption_is_primary "$CREDHUB_KEY_ENCRYPTION_IS_PRIMARY" \
  --arg properties_enable_grootfs "$ENABLE_GROOTFS" \
  --arg properties_enable_service_discovery_for_apps "$ENABLE_SERVICE_DISCOVERY_FOR_APPS" \
  --arg properties_garden_disk_cleanup "$GARDEN_DISK_CLEANUP" \
  --arg properties_gorouter_ssl_ciphers "$GOROUTER_SSL_CIPHERS" \
  --arg properties_haproxy_forward_tls "$HAPROXY_FORWARD_TLS" \
  --arg properties_haproxy_forward_tls_enable_backend_ca "$HAPROXY_FORWARD_TLS_ENABLE_BACKEND_CA" \
  --arg properties_haproxy_hsts_support "$HAPROXY_HSTS_SUPPORT" \
  --arg properties_haproxy_hsts_support_enable_enable_preload "$HAPROXY_HSTS_SUPPORT_ENABLE_ENABLE_PRELOAD" \
  --arg properties_haproxy_hsts_support_enable_include_subdomains "$HAPROXY_HSTS_SUPPORT_ENABLE_INCLUDE_SUBDOMAINS" \
  --arg properties_haproxy_hsts_support_enable_max_age "$HAPROXY_HSTS_SUPPORT_ENABLE_MAX_AGE" \
  --arg properties_haproxy_max_buffer_size "$HAPROXY_MAX_BUFFER_SIZE" \
  --arg properties_haproxy_ssl_ciphers "$HAPROXY_SSL_CIPHERS" \
  --arg properties_logger_endpoint_port "$LOGGER_ENDPOINT_PORT" \
  --arg properties_mysql_activity_logging "$MYSQL_ACTIVITY_LOGGING" \
  --arg properties_mysql_activity_logging_enable_audit_logging_events "$MYSQL_ACTIVITY_LOGGING_ENABLE_AUDIT_LOGGING_EVENTS" \
  --arg properties_networking_poe_ssl_name "$NETWORKING_POE_SSL_NAME" \
  --arg properties_networking_poe_ssl_cert_pem "$NETWORKING_POE_SSL_CERT_PEM" \
  --arg properties_networking_poe_ssl_cert_private_key_pem "$NETWORKING_POE_SSL_CERT_PRIVATE_KEY_PEM" \
  --arg properties_nfs_volume_driver "$NFS_VOLUME_DRIVER" \
  --arg properties_nfs_volume_driver_enable_ldap_server_host "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVER_HOST" \
  --arg properties_nfs_volume_driver_enable_ldap_server_port "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVER_PORT" \
  --arg properties_nfs_volume_driver_enable_ldap_service_account_password "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVICE_ACCOUNT_PASSWORD" \
  --arg properties_nfs_volume_driver_enable_ldap_service_account_user "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVICE_ACCOUNT_USER" \
  --arg properties_nfs_volume_driver_enable_ldap_user_fqdn "$NFS_VOLUME_DRIVER_ENABLE_LDAP_USER_FQDN" \
  --arg properties_push_apps_manager_accent_color "$PUSH_APPS_MANAGER_ACCENT_COLOR" \
  --arg properties_push_apps_manager_company_name "$PUSH_APPS_MANAGER_COMPANY_NAME" \
  --arg properties_push_apps_manager_currency_lookup "$PUSH_APPS_MANAGER_CURRENCY_LOOKUP" \
  --arg properties_push_apps_manager_display_plan_prices "$PUSH_APPS_MANAGER_DISPLAY_PLAN_PRICES" \
  --arg properties_push_apps_manager_enable_invitations "$PUSH_APPS_MANAGER_ENABLE_INVITATIONS" \
  --arg properties_push_apps_manager_favicon "$PUSH_APPS_MANAGER_FAVICON" \
  --arg properties_push_apps_manager_footer_links "$PUSH_APPS_MANAGER_FOOTER_LINKS" \
  --arg properties_push_apps_manager_footer_text "$PUSH_APPS_MANAGER_FOOTER_TEXT" \
  --arg properties_push_apps_manager_global_wrapper_bg_color "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_BG_COLOR" \
  --arg properties_push_apps_manager_global_wrapper_footer_content "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_FOOTER_CONTENT" \
  --arg properties_push_apps_manager_global_wrapper_header_content "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_HEADER_CONTENT" \
  --arg properties_push_apps_manager_global_wrapper_text_color "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_TEXT_COLOR" \
  --arg properties_push_apps_manager_logo "$PUSH_APPS_MANAGER_LOGO" \
  --arg properties_push_apps_manager_marketplace_name "$PUSH_APPS_MANAGER_MARKETPLACE_NAME" \
  --arg properties_push_apps_manager_nav_links "$PUSH_APPS_MANAGER_NAV_LINKS" \
  --arg properties_push_apps_manager_product_name "$PUSH_APPS_MANAGER_PRODUCT_NAME" \
  --arg properties_push_apps_manager_square_logo "$PUSH_APPS_MANAGER_SQUARE_LOGO" \
  --arg properties_rep_proxy_enabled "$REP_PROXY_ENABLED" \
  --arg properties_route_services "$ROUTE_SERVICES" \
  --arg properties_route_services_enable_ignore_ssl_cert_verification "$ROUTE_SERVICES_ENABLE_IGNORE_SSL_CERT_VERIFICATION" \
  --arg properties_router_backend_max_conn "$ROUTER_BACKEND_MAX_CONN" \
  --arg properties_router_client_cert_validation "$ROUTER_CLIENT_CERT_VALIDATION" \
  --arg properties_router_enable_proxy "$ROUTER_ENABLE_PROXY" \
  --arg properties_router_keepalive_connections "$ROUTER_KEEPALIVE_CONNECTIONS" \
  --arg properties_routing_custom_ca_certificates "$ROUTING_CUSTOM_CA_CERTIFICATES" \
  --arg properties_routing_disable_http "$ROUTING_DISABLE_HTTP" \
  --arg properties_routing_minimum_tls_version "$ROUTING_MINIMUM_TLS_VERSION" \
  --arg properties_routing_tls_termination "$ROUTING_TLS_TERMINATION" \
  --arg properties_saml_entity_id_override "$SAML_ENTITY_ID_OVERRIDE" \
  --arg properties_saml_signature_algorithm "$SAML_SIGNATURE_ALGORITHM" \
  --arg properties_secure_service_instance_credentials "$SECURE_SERVICE_INSTANCE_CREDENTIALS" \
  --arg properties_security_acknowledgement "$SECURITY_ACKNOWLEDGEMENT" \
  --arg properties_smoke_tests "$SMOKE_TESTS" \
  --arg properties_smoke_tests_specified_apps_domain "$SMOKE_TESTS_SPECIFIED_APPS_DOMAIN" \
  --arg properties_smoke_tests_specified_org_name "$SMOKE_TESTS_SPECIFIED_ORG_NAME" \
  --arg properties_smoke_tests_specified_space_name "$SMOKE_TESTS_SPECIFIED_SPACE_NAME" \
  --arg properties_smtp_address "$SMTP_ADDRESS" \
  --arg properties_smtp_auth_mechanism "$SMTP_AUTH_MECHANISM" \
  --arg properties_smtp_crammd5_secret "$SMTP_CRAMMD5_SECRET" \
  --arg properties_smtp_user "$SMTP_USER" \
  --arg properties_smtp_password "$SMTP_PASSWORD" \
  --arg properties_smtp_enable_starttls_auto "$SMTP_ENABLE_STARTTLS_AUTO" \
  --arg properties_smtp_from "$SMTP_FROM" \
  --arg properties_smtp_port "$SMTP_PORT" \
  --arg properties_syslog_host "$SYSLOG_HOST" \
  --arg properties_syslog_metrics_to_syslog_enabled "$SYSLOG_METRICS_TO_SYSLOG_ENABLED" \
  --arg properties_syslog_port "$SYSLOG_PORT" \
  --arg properties_syslog_protocol "$SYSLOG_PROTOCOL" \
  --arg properties_syslog_rule "$SYSLOG_RULE" \
  --arg properties_syslog_tls "$SYSLOG_TLS" \
  --arg properties_syslog_tls_enabled_tls_ca_cert "$SYSLOG_TLS_ENABLED_TLS_CA_CERT" \
  --arg properties_syslog_tls_enabled_tls_permitted_peer "$SYSLOG_TLS_ENABLED_TLS_PERMITTED_PEER" \
  --arg properties_syslog_use_tcp_for_file_forwarding_local_transport "$SYSLOG_USE_TCP_FOR_FILE_FORWARDING_LOCAL_TRANSPORT" \
  --arg properties_system_blobstore "$SYSTEM_BLOBSTORE" \
  --arg properties_system_blobstore_external_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_ACCESS_KEY" \
  --arg properties_system_blobstore_external_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_BUILDPACKS_BUCKET" \
  --arg properties_system_blobstore_external_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_DROPLETS_BUCKET" \
  --arg properties_system_blobstore_external_encryption "$SYSTEM_BLOBSTORE_EXTERNAL_ENCRYPTION" \
  --arg properties_system_blobstore_external_encryption_kms_key_id "$SYSTEM_BLOBSTORE_EXTERNAL_ENCRYPTION_KMS_KEY_ID" \
  --arg properties_system_blobstore_external_endpoint "$SYSTEM_BLOBSTORE_EXTERNAL_ENDPOINT" \
  --arg properties_system_blobstore_external_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_PACKAGES_BUCKET" \
  --arg properties_system_blobstore_external_region "$SYSTEM_BLOBSTORE_EXTERNAL_REGION" \
  --arg properties_system_blobstore_external_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_RESOURCES_BUCKET" \
  --arg properties_system_blobstore_external_secret_key "$SYSTEM_BLOBSTORE_EXTERNAL_SECRET_KEY" \
  --arg properties_system_blobstore_external_signature_version "$SYSTEM_BLOBSTORE_EXTERNAL_SIGNATURE_VERSION" \
  --arg properties_system_blobstore_external_versioning "$SYSTEM_BLOBSTORE_EXTERNAL_VERSIONING" \
  --arg properties_system_blobstore_external_azure_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ACCESS_KEY" \
  --arg properties_system_blobstore_external_azure_account_name "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ACCOUNT_NAME" \
  --arg properties_system_blobstore_external_azure_buildpacks_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_BUILDPACKS_CONTAINER" \
  --arg properties_system_blobstore_external_azure_droplets_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_DROPLETS_CONTAINER" \
  --arg properties_system_blobstore_external_azure_environment "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ENVIRONMENT" \
  --arg properties_system_blobstore_external_azure_packages_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_PACKAGES_CONTAINER" \
  --arg properties_system_blobstore_external_azure_resources_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_RESOURCES_CONTAINER" \
  --arg properties_system_blobstore_external_gcs_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_ACCESS_KEY" \
  --arg properties_system_blobstore_external_gcs_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_BUILDPACKS_BUCKET" \
  --arg properties_system_blobstore_external_gcs_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_DROPLETS_BUCKET" \
  --arg properties_system_blobstore_external_gcs_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_PACKAGES_BUCKET" \
  --arg properties_system_blobstore_external_gcs_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_RESOURCES_BUCKET" \
  --arg properties_system_blobstore_external_gcs_secret_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SECRET_KEY" \
  --arg properties_system_blobstore_external_gcs_service_account_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_BUILDPACKS_BUCKET" \
  --arg properties_system_blobstore_external_gcs_service_account_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_DROPLETS_BUCKET" \
  --arg properties_system_blobstore_external_gcs_service_account_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_PACKAGES_BUCKET" \
  --arg properties_system_blobstore_external_gcs_service_account_project_id "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_PROJECT_ID" \
  --arg properties_system_blobstore_external_gcs_service_account_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_RESOURCES_BUCKET" \
  --arg properties_system_blobstore_external_gcs_service_account_service_account_email "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_SERVICE_ACCOUNT_EMAIL" \
  --arg properties_system_blobstore_external_gcs_service_account_service_account_json_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_SERVICE_ACCOUNT_JSON_KEY" \
  --arg properties_system_database "$SYSTEM_DATABASE" \
  --arg properties_system_database_external_account_password "$SYSTEM_DATABASE_EXTERNAL_ACCOUNT_PASSWORD" \
  --arg properties_system_database_external_account_username "$SYSTEM_DATABASE_EXTERNAL_ACCOUNT_USERNAME" \
  --arg properties_system_database_external_app_usage_service_password "$SYSTEM_DATABASE_EXTERNAL_APP_USAGE_SERVICE_PASSWORD" \
  --arg properties_system_database_external_app_usage_service_username "$SYSTEM_DATABASE_EXTERNAL_APP_USAGE_SERVICE_USERNAME" \
  --arg properties_system_database_external_autoscale_password "$SYSTEM_DATABASE_EXTERNAL_AUTOSCALE_PASSWORD" \
  --arg properties_system_database_external_autoscale_username "$SYSTEM_DATABASE_EXTERNAL_AUTOSCALE_USERNAME" \
  --arg properties_system_database_external_ccdb_password "$SYSTEM_DATABASE_EXTERNAL_CCDB_PASSWORD" \
  --arg properties_system_database_external_ccdb_username "$SYSTEM_DATABASE_EXTERNAL_CCDB_USERNAME" \
  --arg properties_system_database_external_diego_password "$SYSTEM_DATABASE_EXTERNAL_DIEGO_PASSWORD" \
  --arg properties_system_database_external_diego_username "$SYSTEM_DATABASE_EXTERNAL_DIEGO_USERNAME" \
  --arg properties_system_database_external_host "$SYSTEM_DATABASE_EXTERNAL_HOST" \
  --arg properties_system_database_external_locket_password "$SYSTEM_DATABASE_EXTERNAL_LOCKET_PASSWORD" \
  --arg properties_system_database_external_locket_username "$SYSTEM_DATABASE_EXTERNAL_LOCKET_USERNAME" \
  --arg properties_system_database_external_networkpolicyserver_password "$SYSTEM_DATABASE_EXTERNAL_NETWORKPOLICYSERVER_PASSWORD" \
  --arg properties_system_database_external_networkpolicyserver_username "$SYSTEM_DATABASE_EXTERNAL_NETWORKPOLICYSERVER_USERNAME" \
  --arg properties_system_database_external_nfsvolume_password "$SYSTEM_DATABASE_EXTERNAL_NFSVOLUME_PASSWORD" \
  --arg properties_system_database_external_nfsvolume_username "$SYSTEM_DATABASE_EXTERNAL_NFSVOLUME_USERNAME" \
  --arg properties_system_database_external_notifications_password "$SYSTEM_DATABASE_EXTERNAL_NOTIFICATIONS_PASSWORD" \
  --arg properties_system_database_external_notifications_username "$SYSTEM_DATABASE_EXTERNAL_NOTIFICATIONS_USERNAME" \
  --arg properties_system_database_external_port "$SYSTEM_DATABASE_EXTERNAL_PORT" \
  --arg properties_system_database_external_routing_password "$SYSTEM_DATABASE_EXTERNAL_ROUTING_PASSWORD" \
  --arg properties_system_database_external_routing_username "$SYSTEM_DATABASE_EXTERNAL_ROUTING_USERNAME" \
  --arg properties_system_database_external_silk_password "$SYSTEM_DATABASE_EXTERNAL_SILK_PASSWORD" \
  --arg properties_system_database_external_silk_username "$SYSTEM_DATABASE_EXTERNAL_SILK_USERNAME" \
  --arg properties_tcp_routing "$TCP_ROUTING" \
  --arg properties_tcp_routing_enable_reservable_ports "$TCP_ROUTING_ENABLE_RESERVABLE_PORTS" \
  --arg properties_uaa "$UAA" \
  --arg properties_uaa_internal_password_expires_after_months "$UAA_INTERNAL_PASSWORD_EXPIRES_AFTER_MONTHS" \
  --arg properties_uaa_internal_password_max_retry "$UAA_INTERNAL_PASSWORD_MAX_RETRY" \
  --arg properties_uaa_internal_password_min_length "$UAA_INTERNAL_PASSWORD_MIN_LENGTH" \
  --arg properties_uaa_internal_password_min_lowercase "$UAA_INTERNAL_PASSWORD_MIN_LOWERCASE" \
  --arg properties_uaa_internal_password_min_numeric "$UAA_INTERNAL_PASSWORD_MIN_NUMERIC" \
  --arg properties_uaa_internal_password_min_special "$UAA_INTERNAL_PASSWORD_MIN_SPECIAL" \
  --arg properties_uaa_internal_password_min_uppercase "$UAA_INTERNAL_PASSWORD_MIN_UPPERCASE" \
  --arg properties_uaa_ldap_identity "$UAA_LDAP_IDENTITY" \
  --arg properties_uaa_ldap_password "$UAA_LDAP_PASSWORD" \
  --arg properties_uaa_ldap_email_domains "$UAA_LDAP_EMAIL_DOMAINS" \
  --arg properties_uaa_ldap_first_name_attribute "$UAA_LDAP_FIRST_NAME_ATTRIBUTE" \
  --arg properties_uaa_ldap_group_search_base "$UAA_LDAP_GROUP_SEARCH_BASE" \
  --arg properties_uaa_ldap_group_search_filter "$UAA_LDAP_GROUP_SEARCH_FILTER" \
  --arg properties_uaa_ldap_last_name_attribute "$UAA_LDAP_LAST_NAME_ATTRIBUTE" \
  --arg properties_uaa_ldap_ldap_referrals "$UAA_LDAP_LDAP_REFERRALS" \
  --arg properties_uaa_ldap_mail_attribute_name "$UAA_LDAP_MAIL_ATTRIBUTE_NAME" \
  --arg properties_uaa_ldap_search_base "$UAA_LDAP_SEARCH_BASE" \
  --arg properties_uaa_ldap_search_filter "$UAA_LDAP_SEARCH_FILTER" \
  --arg properties_uaa_ldap_server_ssl_cert "$UAA_LDAP_SERVER_SSL_CERT" \
  --arg properties_uaa_ldap_server_ssl_cert_alias "$UAA_LDAP_SERVER_SSL_CERT_ALIAS" \
  --arg properties_uaa_ldap_url "$UAA_LDAP_URL" \
  --arg properties_uaa_saml_display_name "$UAA_SAML_DISPLAY_NAME" \
  --arg properties_uaa_saml_email_attribute "$UAA_SAML_EMAIL_ATTRIBUTE" \
  --arg properties_uaa_saml_email_domains "$UAA_SAML_EMAIL_DOMAINS" \
  --arg properties_uaa_saml_entity_id_override "$UAA_SAML_ENTITY_ID_OVERRIDE" \
  --arg properties_uaa_saml_external_groups_attribute "$UAA_SAML_EXTERNAL_GROUPS_ATTRIBUTE" \
  --arg properties_uaa_saml_first_name_attribute "$UAA_SAML_FIRST_NAME_ATTRIBUTE" \
  --arg properties_uaa_saml_last_name_attribute "$UAA_SAML_LAST_NAME_ATTRIBUTE" \
  --arg properties_uaa_saml_name_id_format "$UAA_SAML_NAME_ID_FORMAT" \
  --arg properties_uaa_saml_require_signed_assertions "$UAA_SAML_REQUIRE_SIGNED_ASSERTIONS" \
  --arg properties_uaa_saml_sign_auth_requests "$UAA_SAML_SIGN_AUTH_REQUESTS" \
  --arg properties_uaa_saml_sso_name "$UAA_SAML_SSO_NAME" \
  --arg properties_uaa_saml_sso_url "$UAA_SAML_SSO_URL" \
  --arg properties_uaa_saml_sso_xml "$UAA_SAML_SSO_XML" \
  --arg properties_uaa_database "$UAA_DATABASE" \
  --arg properties_uaa_database_external_host "$UAA_DATABASE_EXTERNAL_HOST" \
  --arg properties_uaa_database_external_port "$UAA_DATABASE_EXTERNAL_PORT" \
  --arg properties_uaa_database_external_uaa_password "$UAA_DATABASE_EXTERNAL_UAA_PASSWORD" \
  --arg properties_uaa_database_external_uaa_username "$UAA_DATABASE_EXTERNAL_UAA_USERNAME" \
  --arg properties_uaa_session_cookie_max_age "$UAA_SESSION_COOKIE_MAX_AGE" \
  --arg properties_uaa_session_idle_timeout "$UAA_SESSION_IDLE_TIMEOUT" \
  ...
  +
  {
    ...
    ".properties.smtp_credentials": {
      "value": {
        "identity": $properties_smtp_user,
        "password": $properties_smtp_password
      }
    },
  ...
  elif $uaa == "ldap" then
  {
    ...
    ".properties.uaa.ldap.credentials": {
      "value":{
        "identity": $properties_uaa_ldap_identity,
        "password": $properties_uaa_ldap_password
      }
    },
