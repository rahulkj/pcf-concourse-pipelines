...
config=$(
  echo "{}" | $JQ_CMD -n \
  --argjson cloud_controller_allow_app_ssh_access "$CLOUD_CONTROLLER_ALLOW_APP_SSH_ACCESS" \
  --argjson cloud_controller_apps_domain "$CLOUD_CONTROLLER_APPS_DOMAIN" \
  --argjson cloud_controller_default_app_memory "$CLOUD_CONTROLLER_DEFAULT_APP_MEMORY" \
  --argjson cloud_controller_default_app_ssh_access "$CLOUD_CONTROLLER_DEFAULT_APP_SSH_ACCESS" \
  --argjson cloud_controller_default_disk_quota_app "$CLOUD_CONTROLLER_DEFAULT_DISK_QUOTA_APP" \
  --argjson cloud_controller_default_quota_max_number_services "$CLOUD_CONTROLLER_DEFAULT_QUOTA_MAX_NUMBER_SERVICES" \
  --argjson cloud_controller_default_quota_memory_limit_mb "$CLOUD_CONTROLLER_DEFAULT_QUOTA_MEMORY_LIMIT_MB" \
  --argjson cloud_controller_enable_custom_buildpacks "$CLOUD_CONTROLLER_ENABLE_CUSTOM_BUILDPACKS" \
  --argjson cloud_controller_encrypt_key "$CLOUD_CONTROLLER_ENCRYPT_KEY" \
  --argjson cloud_controller_max_disk_quota_app "$CLOUD_CONTROLLER_MAX_DISK_QUOTA_APP" \
  --argjson cloud_controller_max_file_size "$CLOUD_CONTROLLER_MAX_FILE_SIZE" \
  --argjson cloud_controller_security_event_logging_enabled "$CLOUD_CONTROLLER_SECURITY_EVENT_LOGGING_ENABLED" \
  --argjson cloud_controller_staging_timeout_in_seconds "$CLOUD_CONTROLLER_STAGING_TIMEOUT_IN_SECONDS" \
  --argjson cloud_controller_system_domain "$CLOUD_CONTROLLER_SYSTEM_DOMAIN" \
  --argjson diego_brain_starting_container_count_maximum "$DIEGO_BRAIN_STARTING_CONTAINER_COUNT_MAXIMUM" \
  --argjson diego_brain_static_ips "$DIEGO_BRAIN_STATIC_IPS" \
  --argjson diego_cell_executor_disk_capacity "$DIEGO_CELL_EXECUTOR_DISK_CAPACITY" \
  --argjson diego_cell_executor_memory_capacity "$DIEGO_CELL_EXECUTOR_MEMORY_CAPACITY" \
  --argjson diego_cell_insecure_docker_registry_list "$DIEGO_CELL_INSECURE_DOCKER_REGISTRY_LIST" \
  --argjson doppler_message_drain_buffer_size "$DOPPLER_MESSAGE_DRAIN_BUFFER_SIZE" \
  --argjson ha_proxy_internal_only_domains "$HA_PROXY_INTERNAL_ONLY_DOMAINS" \
  --argjson ha_proxy_skip_cert_verify "$HA_PROXY_SKIP_CERT_VERIFY" \
  --argjson ha_proxy_static_ips "$HA_PROXY_STATIC_IPS" \
  --argjson ha_proxy_trusted_domain_cidrs "$HA_PROXY_TRUSTED_DOMAIN_CIDRS" \
  --argjson mysql_cli_history "$MYSQL_CLI_HISTORY" \
  --argjson mysql_cluster_probe_timeout "$MYSQL_CLUSTER_PROBE_TIMEOUT" \
  --argjson mysql_prevent_node_auto_rejoin "$MYSQL_PREVENT_NODE_AUTO_REJOIN" \
  --argjson mysql_remote_admin_access "$MYSQL_REMOTE_ADMIN_ACCESS" \
  --argjson mysql_monitor_poll_frequency "$MYSQL_MONITOR_POLL_FREQUENCY" \
  --argjson mysql_monitor_recipient_email "$MYSQL_MONITOR_RECIPIENT_EMAIL" \
  --argjson mysql_monitor_write_read_delay "$MYSQL_MONITOR_WRITE_READ_DELAY" \
  --argjson mysql_proxy_service_hostname "$MYSQL_PROXY_SERVICE_HOSTNAME" \
  --argjson mysql_proxy_shutdown_delay "$MYSQL_PROXY_SHUTDOWN_DELAY" \
  --argjson mysql_proxy_startup_delay "$MYSQL_PROXY_STARTUP_DELAY" \
  --argjson mysql_proxy_static_ips "$MYSQL_PROXY_STATIC_IPS" \
  --argjson nfs_server_blobstore_internal_access_rules ""$NFS_SERVER_BLOBSTORE_INTERNAL_ACCESS_RULES"" \
  --argjson autoscale_api_instance_count "$AUTOSCALE_API_INSTANCE_COUNT" \
  --argjson autoscale_instance_count "$AUTOSCALE_INSTANCE_COUNT" \
  --argjson autoscale_metric_bucket_count "$AUTOSCALE_METRIC_BUCKET_COUNT" \
  --argjson autoscale_scaling_interval_in_seconds "$AUTOSCALE_SCALING_INTERVAL_IN_SECONDS" \
  --argjson cc_api_rate_limit "$CC_API_RATE_LIMIT" \
  --argjson cc_api_rate_limit_enable_general_limit "$CC_API_RATE_LIMIT_ENABLE_GENERAL_LIMIT" \
  --argjson cc_api_rate_limit_enable_unauthenticated_limit "$CC_API_RATE_LIMIT_ENABLE_UNAUTHENTICATED_LIMIT" \
  --argjson cf_dial_timeout_in_seconds "$CF_DIAL_TIMEOUT_IN_SECONDS" \
  --argjson cf_networking_enable_space_developer_self_service "$CF_NETWORKING_ENABLE_SPACE_DEVELOPER_SELF_SERVICE" \
  --argjson container_networking "$CONTAINER_NETWORKING" \
  --argjson container_networking_interface_plugin "$CONTAINER_NETWORKING_INTERFACE_PLUGIN" \
  --argjson container_networking_interface_plugin_silk_dns_servers "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_DNS_SERVERS" \
  --argjson container_networking_interface_plugin_silk_enable_log_traffic "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_ENABLE_LOG_TRAFFIC" \
  --argjson container_networking_interface_plugin_silk_iptables_accepted_udp_logs_per_sec "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_IPTABLES_ACCEPTED_UDP_LOGS_PER_SEC" \
  --argjson container_networking_interface_plugin_silk_iptables_denied_logs_per_sec "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_IPTABLES_DENIED_LOGS_PER_SEC" \
  --argjson container_networking_interface_plugin_silk_network_cidr "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_NETWORK_CIDR" \
  --argjson container_networking_interface_plugin_silk_network_mtu "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_NETWORK_MTU" \
  --argjson container_networking_interface_plugin_silk_vtep_port "$CONTAINER_NETWORKING_INTERFACE_PLUGIN_SILK_VTEP_PORT" \
  --argjson credhub_database "$CREDHUB_DATABASE" \
  --argjson credhub_database_external_host "$CREDHUB_DATABASE_EXTERNAL_HOST" \
  --argjson credhub_database_external_password "$CREDHUB_DATABASE_EXTERNAL_PASSWORD" \
  --argjson credhub_database_external_port "$CREDHUB_DATABASE_EXTERNAL_PORT" \
  --argjson credhub_database_external_tls_ca "$CREDHUB_DATABASE_EXTERNAL_TLS_CA" \
  --argjson credhub_database_external_username "$CREDHUB_DATABASE_EXTERNAL_USERNAME" \
  --argjson credhub_hsm_provider_client_certificate "$CREDHUB_HSM_PROVIDER_CLIENT_CERTIFICATE" \
  ...
  --argjson credhub_key_encryption_name "$CREDHUB_KEY_ENCRYPTION_NAME" \
  --argjson credhub_key_encryption_secret "$CREDHUB_KEY_ENCRYPTION_SECRET" \
  --argjson credhub_key_encryption_is_primary "$CREDHUB_KEY_ENCRYPTION_IS_PRIMARY" \
  --argjson enable_grootfs "$ENABLE_GROOTFS" \
  --argjson enable_service_discovery_for_apps "$ENABLE_SERVICE_DISCOVERY_FOR_APPS" \
  --argjson garden_disk_cleanup "$GARDEN_DISK_CLEANUP" \
  --argjson gorouter_ssl_ciphers "$GOROUTER_SSL_CIPHERS" \
  --argjson haproxy_forward_tls "$HAPROXY_FORWARD_TLS" \
  --argjson haproxy_forward_tls_enable_backend_ca "$HAPROXY_FORWARD_TLS_ENABLE_BACKEND_CA" \
  --argjson haproxy_hsts_support "$HAPROXY_HSTS_SUPPORT" \
  --argjson haproxy_hsts_support_enable_enable_preload "$HAPROXY_HSTS_SUPPORT_ENABLE_ENABLE_PRELOAD" \
  --argjson haproxy_hsts_support_enable_include_subdomains "$HAPROXY_HSTS_SUPPORT_ENABLE_INCLUDE_SUBDOMAINS" \
  --argjson haproxy_hsts_support_enable_max_age "$HAPROXY_HSTS_SUPPORT_ENABLE_MAX_AGE" \
  --argjson haproxy_max_buffer_size "$HAPROXY_MAX_BUFFER_SIZE" \
  --argjson haproxy_ssl_ciphers "$HAPROXY_SSL_CIPHERS" \
  --argjson logger_endpoint_port "$LOGGER_ENDPOINT_PORT" \
  --argjson mysql_activity_logging "$MYSQL_ACTIVITY_LOGGING" \
  --argjson mysql_activity_logging_enable_audit_logging_events "$MYSQL_ACTIVITY_LOGGING_ENABLE_AUDIT_LOGGING_EVENTS" \
  --argjson networking_poe_ssl_name "$NETWORKING_POE_SSL_NAME" \
  --argjson networking_poe_ssl_cert_pem "$NETWORKING_POE_SSL_CERT_PEM" \
  --argjson networking_poe_ssl_cert_private_key_pem "$NETWORKING_POE_SSL_CERT_PRIVATE_KEY_PEM" \
  --argjson nfs_volume_driver "$NFS_VOLUME_DRIVER" \
  --argjson nfs_volume_driver_enable_ldap_server_host "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVER_HOST" \
  --argjson nfs_volume_driver_enable_ldap_server_port "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVER_PORT" \
  --argjson nfs_volume_driver_enable_ldap_service_account_password "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVICE_ACCOUNT_PASSWORD" \
  --argjson nfs_volume_driver_enable_ldap_service_account_user "$NFS_VOLUME_DRIVER_ENABLE_LDAP_SERVICE_ACCOUNT_USER" \
  --argjson nfs_volume_driver_enable_ldap_user_fqdn "$NFS_VOLUME_DRIVER_ENABLE_LDAP_USER_FQDN" \
  --argjson push_apps_manager_accent_color "$PUSH_APPS_MANAGER_ACCENT_COLOR" \
  --argjson push_apps_manager_company_name "$PUSH_APPS_MANAGER_COMPANY_NAME" \
  --argjson push_apps_manager_currency_lookup "$PUSH_APPS_MANAGER_CURRENCY_LOOKUP" \
  --argjson push_apps_manager_display_plan_prices "$PUSH_APPS_MANAGER_DISPLAY_PLAN_PRICES" \
  --argjson push_apps_manager_enable_invitations "$PUSH_APPS_MANAGER_ENABLE_INVITATIONS" \
  --argjson push_apps_manager_favicon "$PUSH_APPS_MANAGER_FAVICON" \
  --argjson push_apps_manager_footer_links "$PUSH_APPS_MANAGER_FOOTER_LINKS" \
  --argjson push_apps_manager_footer_text "$PUSH_APPS_MANAGER_FOOTER_TEXT" \
  --argjson push_apps_manager_global_wrapper_bg_color "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_BG_COLOR" \
  --argjson push_apps_manager_global_wrapper_footer_content "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_FOOTER_CONTENT" \
  --argjson push_apps_manager_global_wrapper_header_content "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_HEADER_CONTENT" \
  --argjson push_apps_manager_global_wrapper_text_color "$PUSH_APPS_MANAGER_GLOBAL_WRAPPER_TEXT_COLOR" \
  --argjson push_apps_manager_logo "$PUSH_APPS_MANAGER_LOGO" \
  --argjson push_apps_manager_marketplace_name "$PUSH_APPS_MANAGER_MARKETPLACE_NAME" \
  --argjson push_apps_manager_nav_link_name_1 "$PUSH_APPS_MANAGER_NAV_LINK_NAME_1" \
  --argjson push_apps_manager_nav_link_href_1 "$PUSH_APPS_MANAGER_NAV_LINK_HREF_1" \
  --argjson push_apps_manager_nav_link_name_2 "$PUSH_APPS_MANAGER_NAV_LINK_NAME_2" \
  --argjson push_apps_manager_nav_link_href_2 "$PUSH_APPS_MANAGER_NAV_LINK_HREF_2" \
  --argjson push_apps_manager_nav_link_name_3 "$PUSH_APPS_MANAGER_NAV_LINK_NAME_3" \
  --argjson push_apps_manager_nav_link_href_3 "$PUSH_APPS_MANAGER_NAV_LINK_HREF_3" \
  --argjson push_apps_manager_product_name "$PUSH_APPS_MANAGER_PRODUCT_NAME" \
  --argjson push_apps_manager_square_logo "$PUSH_APPS_MANAGER_SQUARE_LOGO" \
  --argjson rep_proxy_enabled "$REP_PROXY_ENABLED" \
  --argjson route_services "$ROUTE_SERVICES" \
  --argjson route_services_enable_ignore_ssl_cert_verification "$ROUTE_SERVICES_ENABLE_IGNORE_SSL_CERT_VERIFICATION" \
  --argjson router_backend_max_conn "$ROUTER_BACKEND_MAX_CONN" \
  --argjson router_client_cert_validation "$ROUTER_CLIENT_CERT_VALIDATION" \
  --argjson router_enable_proxy "$ROUTER_ENABLE_PROXY" \
  --argjson router_keepalive_connections "$ROUTER_KEEPALIVE_CONNECTIONS" \
  --argjson routing_custom_ca_certificates "$ROUTING_CUSTOM_CA_CERTIFICATES" \
  --argjson routing_disable_http "$ROUTING_DISABLE_HTTP" \
  --argjson routing_minimum_tls_version "$ROUTING_MINIMUM_TLS_VERSION" \
  --argjson routing_tls_termination "$ROUTING_TLS_TERMINATION" \
  --argjson saml_entity_id_override "$SAML_ENTITY_ID_OVERRIDE" \
  --argjson saml_signature_algorithm "$SAML_SIGNATURE_ALGORITHM" \
  --argjson secure_service_instance_credentials "$SECURE_SERVICE_INSTANCE_CREDENTIALS" \
  --argjson security_acknowledgement "$SECURITY_ACKNOWLEDGEMENT" \
  --argjson smoke_tests "$SMOKE_TESTS" \
  --argjson smoke_tests_specified_apps_domain "$SMOKE_TESTS_SPECIFIED_APPS_DOMAIN" \
  --argjson smoke_tests_specified_org_name "$SMOKE_TESTS_SPECIFIED_ORG_NAME" \
  --argjson smoke_tests_specified_space_name "$SMOKE_TESTS_SPECIFIED_SPACE_NAME" \
  --argjson smtp_address "$SMTP_ADDRESS" \
  --argjson smtp_auth_mechanism "$SMTP_AUTH_MECHANISM" \
  --argjson smtp_crammd5_secret "$SMTP_CRAMMD5_SECRET" \
  --argjson smtp_user "$SMTP_USER" \
  --argjson smtp_password "$SMTP_PASSWORD" \
  --argjson smtp_enable_starttls_auto "$SMTP_ENABLE_STARTTLS_AUTO" \
  --argjson smtp_from "$SMTP_FROM" \
  --argjson smtp_port "$SMTP_PORT" \
  --argjson syslog_host "$SYSLOG_HOST" \
  --argjson syslog_metrics_to_syslog_enabled "$SYSLOG_METRICS_TO_SYSLOG_ENABLED" \
  --argjson syslog_port "$SYSLOG_PORT" \
  --argjson syslog_protocol "$SYSLOG_PROTOCOL" \
  --argjson syslog_rule "$SYSLOG_RULE" \
  --argjson syslog_tls "$SYSLOG_TLS" \
  --argjson syslog_tls_enabled_tls_ca_cert "$SYSLOG_TLS_ENABLED_TLS_CA_CERT" \
  --argjson syslog_tls_enabled_tls_permitted_peer "$SYSLOG_TLS_ENABLED_TLS_PERMITTED_PEER" \
  --argjson syslog_use_tcp_for_file_forwarding_local_transport "$SYSLOG_USE_TCP_FOR_FILE_FORWARDING_LOCAL_TRANSPORT" \
  --argjson system_blobstore "$SYSTEM_BLOBSTORE" \
  --argjson system_blobstore_external_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_ACCESS_KEY" \
  --argjson system_blobstore_external_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_BUILDPACKS_BUCKET" \
  --argjson system_blobstore_external_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_DROPLETS_BUCKET" \
  --argjson system_blobstore_external_encryption "$SYSTEM_BLOBSTORE_EXTERNAL_ENCRYPTION" \
  --argjson system_blobstore_external_encryption_kms_key_id "$SYSTEM_BLOBSTORE_EXTERNAL_ENCRYPTION_KMS_KEY_ID" \
  --argjson system_blobstore_external_endpoint "$SYSTEM_BLOBSTORE_EXTERNAL_ENDPOINT" \
  --argjson system_blobstore_external_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_PACKAGES_BUCKET" \
  --argjson system_blobstore_external_region "$SYSTEM_BLOBSTORE_EXTERNAL_REGION" \
  --argjson system_blobstore_external_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_RESOURCES_BUCKET" \
  --argjson system_blobstore_external_secret_key "$SYSTEM_BLOBSTORE_EXTERNAL_SECRET_KEY" \
  --argjson system_blobstore_external_signature_version "$SYSTEM_BLOBSTORE_EXTERNAL_SIGNATURE_VERSION" \
  --argjson system_blobstore_external_versioning "$SYSTEM_BLOBSTORE_EXTERNAL_VERSIONING" \
  --argjson system_blobstore_external_azure_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ACCESS_KEY" \
  --argjson system_blobstore_external_azure_account_name "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ACCOUNT_NAME" \
  --argjson system_blobstore_external_azure_buildpacks_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_BUILDPACKS_CONTAINER" \
  --argjson system_blobstore_external_azure_droplets_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_DROPLETS_CONTAINER" \
  --argjson system_blobstore_external_azure_environment "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_ENVIRONMENT" \
  --argjson system_blobstore_external_azure_packages_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_PACKAGES_CONTAINER" \
  --argjson system_blobstore_external_azure_resources_container "$SYSTEM_BLOBSTORE_EXTERNAL_AZURE_RESOURCES_CONTAINER" \
  --argjson system_blobstore_external_gcs_access_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_ACCESS_KEY" \
  --argjson system_blobstore_external_gcs_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_BUILDPACKS_BUCKET" \
  --argjson system_blobstore_external_gcs_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_DROPLETS_BUCKET" \
  --argjson system_blobstore_external_gcs_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_PACKAGES_BUCKET" \
  --argjson system_blobstore_external_gcs_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_RESOURCES_BUCKET" \
  --argjson system_blobstore_external_gcs_secret_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SECRET_KEY" \
  --argjson system_blobstore_external_gcs_service_account_buildpacks_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_BUILDPACKS_BUCKET" \
  --argjson system_blobstore_external_gcs_service_account_droplets_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_DROPLETS_BUCKET" \
  --argjson system_blobstore_external_gcs_service_account_packages_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_PACKAGES_BUCKET" \
  --argjson system_blobstore_external_gcs_service_account_project_id "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_PROJECT_ID" \
  --argjson system_blobstore_external_gcs_service_account_resources_bucket "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_RESOURCES_BUCKET" \
  --argjson system_blobstore_external_gcs_service_account_service_account_email "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_SERVICE_ACCOUNT_EMAIL" \
  --argjson system_blobstore_external_gcs_service_account_service_account_json_key "$SYSTEM_BLOBSTORE_EXTERNAL_GCS_SERVICE_ACCOUNT_SERVICE_ACCOUNT_JSON_KEY" \
  --argjson system_database "$SYSTEM_DATABASE" \
  --argjson system_database_external_account_password "$SYSTEM_DATABASE_EXTERNAL_ACCOUNT_PASSWORD" \
  --argjson system_database_external_account_username "$SYSTEM_DATABASE_EXTERNAL_ACCOUNT_USERNAME" \
  --argjson system_database_external_app_usage_service_password "$SYSTEM_DATABASE_EXTERNAL_APP_USAGE_SERVICE_PASSWORD" \
  --argjson system_database_external_app_usage_service_username "$SYSTEM_DATABASE_EXTERNAL_APP_USAGE_SERVICE_USERNAME" \
  --argjson system_database_external_autoscale_password "$SYSTEM_DATABASE_EXTERNAL_AUTOSCALE_PASSWORD" \
  --argjson system_database_external_autoscale_username "$SYSTEM_DATABASE_EXTERNAL_AUTOSCALE_USERNAME" \
  --argjson system_database_external_ccdb_password "$SYSTEM_DATABASE_EXTERNAL_CCDB_PASSWORD" \
  --argjson system_database_external_ccdb_username "$SYSTEM_DATABASE_EXTERNAL_CCDB_USERNAME" \
  --argjson system_database_external_diego_password "$SYSTEM_DATABASE_EXTERNAL_DIEGO_PASSWORD" \
  --argjson system_database_external_diego_username "$SYSTEM_DATABASE_EXTERNAL_DIEGO_USERNAME" \
  --argjson system_database_external_host "$SYSTEM_DATABASE_EXTERNAL_HOST" \
  --argjson system_database_external_locket_password "$SYSTEM_DATABASE_EXTERNAL_LOCKET_PASSWORD" \
  --argjson system_database_external_locket_username "$SYSTEM_DATABASE_EXTERNAL_LOCKET_USERNAME" \
  --argjson system_database_external_networkpolicyserver_password "$SYSTEM_DATABASE_EXTERNAL_NETWORKPOLICYSERVER_PASSWORD" \
  --argjson system_database_external_networkpolicyserver_username "$SYSTEM_DATABASE_EXTERNAL_NETWORKPOLICYSERVER_USERNAME" \
  --argjson system_database_external_nfsvolume_password "$SYSTEM_DATABASE_EXTERNAL_NFSVOLUME_PASSWORD" \
  --argjson system_database_external_nfsvolume_username "$SYSTEM_DATABASE_EXTERNAL_NFSVOLUME_USERNAME" \
  --argjson system_database_external_notifications_password "$SYSTEM_DATABASE_EXTERNAL_NOTIFICATIONS_PASSWORD" \
  --argjson system_database_external_notifications_username "$SYSTEM_DATABASE_EXTERNAL_NOTIFICATIONS_USERNAME" \
  --argjson system_database_external_port "$SYSTEM_DATABASE_EXTERNAL_PORT" \
  --argjson system_database_external_routing_password "$SYSTEM_DATABASE_EXTERNAL_ROUTING_PASSWORD" \
  --argjson system_database_external_routing_username "$SYSTEM_DATABASE_EXTERNAL_ROUTING_USERNAME" \
  --argjson system_database_external_silk_password "$SYSTEM_DATABASE_EXTERNAL_SILK_PASSWORD" \
  --argjson system_database_external_silk_username "$SYSTEM_DATABASE_EXTERNAL_SILK_USERNAME" \
  --argjson tcp_routing "$TCP_ROUTING" \
  --argjson tcp_routing_enable_reservable_ports "$TCP_ROUTING_ENABLE_RESERVABLE_PORTS" \
  --argjson uaa "$UAA" \
  --argjson uaa_internal_password_expires_after_months "$UAA_INTERNAL_PASSWORD_EXPIRES_AFTER_MONTHS" \
  --argjson uaa_internal_password_max_retry "$UAA_INTERNAL_PASSWORD_MAX_RETRY" \
  --argjson uaa_internal_password_min_length "$UAA_INTERNAL_PASSWORD_MIN_LENGTH" \
  --argjson uaa_internal_password_min_lowercase "$UAA_INTERNAL_PASSWORD_MIN_LOWERCASE" \
  --argjson uaa_internal_password_min_numeric "$UAA_INTERNAL_PASSWORD_MIN_NUMERIC" \
  --argjson uaa_internal_password_min_special "$UAA_INTERNAL_PASSWORD_MIN_SPECIAL" \
  --argjson uaa_internal_password_min_uppercase "$UAA_INTERNAL_PASSWORD_MIN_UPPERCASE" \
  --argjson uaa_ldap_identity "$UAA_LDAP_IDENTITY" \
  --argjson uaa_ldap_password "$UAA_LDAP_PASSWORD" \
  --argjson uaa_ldap_email_domains "$UAA_LDAP_EMAIL_DOMAINS" \
  --argjson uaa_ldap_first_name_attribute "$UAA_LDAP_FIRST_NAME_ATTRIBUTE" \
  --argjson uaa_ldap_group_search_base "$UAA_LDAP_GROUP_SEARCH_BASE" \
  --argjson uaa_ldap_group_search_filter "$UAA_LDAP_GROUP_SEARCH_FILTER" \
  --argjson uaa_ldap_last_name_attribute "$UAA_LDAP_LAST_NAME_ATTRIBUTE" \
  --argjson uaa_ldap_ldap_referrals "$UAA_LDAP_LDAP_REFERRALS" \
  --argjson uaa_ldap_mail_attribute_name "$UAA_LDAP_MAIL_ATTRIBUTE_NAME" \
  --argjson uaa_ldap_search_base "$UAA_LDAP_SEARCH_BASE" \
  --argjson uaa_ldap_search_filter "$UAA_LDAP_SEARCH_FILTER" \
  --argjson uaa_ldap_server_ssl_cert "$UAA_LDAP_SERVER_SSL_CERT" \
  --argjson uaa_ldap_server_ssl_cert_alias "$UAA_LDAP_SERVER_SSL_CERT_ALIAS" \
  --argjson uaa_ldap_url "$UAA_LDAP_URL" \
  --argjson uaa_saml_display_name "$UAA_SAML_DISPLAY_NAME" \
  --argjson uaa_saml_email_attribute "$UAA_SAML_EMAIL_ATTRIBUTE" \
  --argjson uaa_saml_email_domains "$UAA_SAML_EMAIL_DOMAINS" \
  --argjson uaa_saml_entity_id_override "$UAA_SAML_ENTITY_ID_OVERRIDE" \
  --argjson uaa_saml_external_groups_attribute "$UAA_SAML_EXTERNAL_GROUPS_ATTRIBUTE" \
  --argjson uaa_saml_first_name_attribute "$UAA_SAML_FIRST_NAME_ATTRIBUTE" \
  --argjson uaa_saml_last_name_attribute "$UAA_SAML_LAST_NAME_ATTRIBUTE" \
  --argjson uaa_saml_name_id_format "$UAA_SAML_NAME_ID_FORMAT" \
  --argjson uaa_saml_require_signed_assertions "$UAA_SAML_REQUIRE_SIGNED_ASSERTIONS" \
  --argjson uaa_saml_sign_auth_requests "$UAA_SAML_SIGN_AUTH_REQUESTS" \
  --argjson uaa_saml_sso_name "$UAA_SAML_SSO_NAME" \
  --argjson uaa_saml_sso_url "$UAA_SAML_SSO_URL" \
  --argjson uaa_saml_sso_xml "$UAA_SAML_SSO_XML" \
  --argjson uaa_database "$UAA_DATABASE" \
  --argjson uaa_database_external_host "$UAA_DATABASE_EXTERNAL_HOST" \
  --argjson uaa_database_external_port "$UAA_DATABASE_EXTERNAL_PORT" \
  --argjson uaa_database_external_uaa_password "$UAA_DATABASE_EXTERNAL_UAA_PASSWORD" \
  --argjson uaa_database_external_uaa_username "$UAA_DATABASE_EXTERNAL_UAA_USERNAME" \
  --argjson uaa_session_cookie_max_age "$UAA_SESSION_COOKIE_MAX_AGE" \
  --argjson uaa_session_idle_timeout "$UAA_SESSION_IDLE_TIMEOUT" \
  --argjson router_disable_insecure_cookies "$ROUTER_DISABLE_INSECURE_COOKIES" \
  --argjson router_drain_wait "$ROUTER_DRAIN_WAIT" \
  --argjson router_enable_isolated_routing "$ROUTER_ENABLE_ISOLATED_ROUTING" \
  --argjson router_enable_write_access_logs "$ROUTER_ENABLE_WRITE_ACCESS_LOGS" \
  --argjson router_enable_zipkin "$ROUTER_ENABLE_ZIPKIN" \
  --argjson router_extra_headers_to_log "$ROUTER_EXTRA_HEADERS_TO_LOG" \
  --argjson router_frontend_idle_timeout "$ROUTER_FRONTEND_IDLE_TIMEOUT" \
  --argjson router_lb_healthy_threshold "$ROUTER_LB_HEALTHY_THRESHOLD" \
  --argjson router_request_timeout_in_seconds "$ROUTER_REQUEST_TIMEOUT_IN_SECONDS" \
  --argjson router_static_ips "$ROUTER_STATIC_IPS" \
  --argjson tcp_router_static_ips "$TCP_ROUTER_STATIC_IPS" \
  --argjson uaa_apps_manager_access_token_lifetime "$UAA_APPS_MANAGER_ACCESS_TOKEN_LIFETIME" \
  --argjson uaa_apps_manager_refresh_token_lifetime "$UAA_APPS_MANAGER_REFRESH_TOKEN_LIFETIME" \
  --argjson uaa_cf_cli_access_token_lifetime "$UAA_CF_CLI_ACCESS_TOKEN_LIFETIME" \
  --argjson uaa_cf_cli_refresh_token_lifetime "$UAA_CF_CLI_REFRESH_TOKEN_LIFETIME" \
  --argjson uaa_customize_password_label "$UAA_CUSTOMIZE_PASSWORD_LABEL" \
  --argjson uaa_customize_username_label "$UAA_CUSTOMIZE_USERNAME_LABEL" \
  --argjson uaa_issuer_uri "$UAA_ISSUER_URI" \
  --argjson uaa_proxy_ips_regex "$UAA_PROXY_IPS_REGEX" \
  --argjson uaa_private_key_pem "$UAA_PRIVATE_KEY_PEM" \
  --argjson uaa_cert_pem "$UAA_CERT_PEM" \
  --argjson uaa_service_provider_key_password "$UAA_SERVICE_PROVIDER_KEY_PASSWORD" \
  '
  . +
  {
