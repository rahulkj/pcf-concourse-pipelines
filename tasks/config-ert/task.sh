...
CF_RELEASE=$($OM_CMD -k available-products -f json)
...
$OM_CMD -k stage-product -p $PRODUCT_NAME -v $PRODUCT_VERSION
...
EOF
  ...
  CERTIFICATES=`$OM_CMD -k curl -s -p "/api/v0/certificates/generate" -x POST -d "$DOMAINS"`
  ...
  CERTIFICATES=`$OM_CMD -k curl -s -p "/api/v0/certificates/generate" -x POST -d "$SECURITY_DOMAIN"`
