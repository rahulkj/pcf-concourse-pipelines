...
config=$($JQ_CMD -n \
  --argjson add_app_info ${ADD_APP_INFO:-false} \
  --argjson allow_paid_service_plans ${ALLOW_PAID_SERVICE_PLANS:-true} \
  ...
  --argjson enable_event_tracing ${ENABLE_EVENT_TRACING:-false} \
  --arg events $EVENTS \
  --arg extra_fields $EXTRA_FIELDS \
  --arg firehose_subscription_id $FIREHOSE_SUBSCRIPTION_ID \
  --arg org ${ORG:-"splunk-nozzle-org"} \
  --argjson scale_out_nozzle ${SCALE_OUT_NOZZLE:-2} \
  --argjson skip_ssl_validation_cf ${SKIP_SSL_VALIDATION_CF:-false} \
  --argjson skip_ssl_validation_splunk ${SKIP_SSL_VALIDATION_SPLUNK:-false} \
  --arg space ${SPACE:-"splunk-nozzle-space"} \
  --arg splunk_host $SPLUNK_HOST \
  --arg splunk_index $SPLUNK_INDEX \
